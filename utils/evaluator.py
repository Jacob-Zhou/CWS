...
class Decoder(object):
    ...
    def __init__(self):
        inf = float('-Inf')
        T = [[0.,inf,inf,0.],[inf,0.,0.,inf],[inf,inf,0.,inf],[0.,inf,inf,0.],[0.,inf,inf,0.]]
    ...
    def viterbi(self, score, labelSize):
         ...
         for idx in range(length):
             if idx == 0:
                 ...
                 viterbi_score[idx] = score[idx] + torch.max(self.gram[1:] + viterbi_score[idx-1],1)[0]
...
#            targets = torch.split(targets[mask], sen_lens.tolist())
            ...
            for predict, target in zip(predicts, targets):
                ...
                overall_count, predict_count, correct_count = Decoder().eval(predict, target)
