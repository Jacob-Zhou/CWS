...
def drop_input_word_tag_emb_independent(word_embeddings, tag_embeddings, dropout_ratio):
    assert (dropout_ratio >= 0.33 - 1e-5) and dropout_ratio <= (0.33 + 1e-5)
    batch_size, seq_length, _ = word_embeddings.size()
    word_masks = compose_drop_mask(
        word_embeddings, (batch_size, seq_length), dropout_ratio)
    tag_masks = compose_drop_mask(
        tag_embeddings, (batch_size, seq_length), dropout_ratio)
...
def compose_drop_mask(x, size, dropout_ratio):
    ...
    drop_mask = x.new_full(size, 1 - dropout_ratio, requires_grad=False)
...
def drop_sequence_shared_mask(inputs, dropout_ratio):
    seq_length, batch_size, hidden_size = inputs.size()
    drop_masks = compose_drop_mask(
        inputs, (batch_size, hidden_size), dropout_ratio) / (1 - dropout_ratio)
