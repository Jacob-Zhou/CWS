...
# $exe --device 3 --config config_zx.bert.ini --path=exp/zx.cd.bert --is-dictionary-exist --is-train > results/log.zx.cd.bert 2>&1 
$exe --device 3 --config config_zx_test.bert.ini --path=exp/zx.cd.bert --is-test > results/log.test.zx.cd.bert 2>&1 &